      ...
      if (!posterQuota.hasQuota) {
        ...
        if (posterQuota.planType === 'trial') {
          message = MessageTemplates.createTextMessage(
            ...
            `スタンダードプランにアップグレードすると、無制限でポスターを作成できます！✨\n\n` +
            `アップグレードをご希望の場合は、下部メニューの「クーポン配布中！」からお手続きください。`
          );
        } else {
          // 无订阅用户
          message = MessageTemplates.createTextMessage(
            `📸 ポスター作成機能をご利用いただくには、プランへの加入が必要です。\n\n` +
            `• トライアルプラン: 8枚/月\n` +
            `• スタンダードプラン: 無制限 ♾️\n\n` +
  ...
  async executePosterGenerationWithPolling(replyToken, user, imageUrl, posterTaskId) {
    ...
    try {
      ...
      try {
        ...
        console.error('❌ 标记任务失败时出错:', dbError);
      }
  ...
  async handleCheckStatus(event, user) {
    try {
      ...
      if (activePosterTask) {
        ...
        console.log(`📸 发现活跃海报任务，运行时间: ${Math.floor(elapsedTime/1000)}秒`);
