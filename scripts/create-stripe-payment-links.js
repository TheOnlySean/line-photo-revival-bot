  ...
  try {
    ...
    const trialPaymentLink = await stripe.paymentLinks.create({
      ...
      after_completion: {
        type: 'redirect',
        redirect: {
          url: `${baseUrl}/api/subscription/success?plan=trial&user_id={CHECKOUT_SESSION_ID}`
        }
      },
      metadata: {
        plan_type: 'trial',
    ...
    const standardPaymentLink = await stripe.paymentLinks.create({
      ...
      after_completion: {
        type: 'redirect',
        redirect: {
          url: `${baseUrl}/api/subscription/success?plan=standard&user_id={CHECKOUT_SESSION_ID}`
        }
      },
      metadata: {
        plan_type: 'standard',
